...
foreach ( $events as $post ) {
    setup_postdata( $post );
        ...
        echo '</div>';
        echo '<div id="upcoming-banner-meta-'.$counter.'">';
        echo '<p id="upcoming-banner-title-'.$counter.'">'.$post->post_title.'<p>';
        echo '<p id="upcoming-banner-date-'.$counter.'">'.tribe_get_start_date($post).'</p>';
...
<script>
	for(i = 1; i <= 3; i++){
		var card_front = document.getElementById("front-" + i);
		var card_back = document.getElementById("back-" + i);
		...
		card_back.appendChild(event_meta);
	}
</script>
