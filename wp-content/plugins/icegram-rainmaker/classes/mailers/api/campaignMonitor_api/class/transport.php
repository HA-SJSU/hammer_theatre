...
if(!function_exists("Rm_REST_TRANSPORT_get_available")) {    
    function Rm_REST_TRANSPORT_get_available($requires_ssl, $log) {
        ...
        } else if(Rm_REST_TRANSPORT_can_use_raw_socket($requires_ssl)) {
            return new Rm_REST_SocketTransport($log);
        } else { 
            $log->log_message('No transport is available', __FUNCTION__, Rm_REST_LOG_ERROR);
...
if (!class_exists('Rm_REST_CurlTransport')) {
    class Rm_REST_CurlTransport extends Rm_REST_BaseTransport {
        ...
        function make_call($call_options) {
            ...
            curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, Rm_REST_SOCKET_TIMEOUT);
            curl_setopt($ch, CURLOPT_TIMEOUT, Rm_REST_CALL_TIMEOUT);
            ...
            switch($call_options['method']) {
                case Rm_REST_PUT:
                    curl_setopt($ch, CURLOPT_CUSTOMREQUEST, Rm_REST_PUT);
                ...
                case Rm_REST_POST:
                    ...
                    break;
                case Rm_REST_DELETE:
                    curl_setopt($ch, CURLOPT_CUSTOMREQUEST, Rm_REST_DELETE);
                    break;
            }
