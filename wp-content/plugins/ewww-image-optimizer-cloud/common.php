	...
	if ( is_wp_error( $result ) ) {
		...
		if ( 0 == $quota[0] && $quota[1] > 0 ) {
			return esc_html( sprintf(
				...
				_n( 'optimized %1$d images, usage will reset in %2$d day.', 'optimized %1$d images, usage will reset in %2$d days.', $quota[2], 'ewww-image-optimizer-cloud' ), $quota[1], $quota[2]
			) );
		} elseif ( 0 == $quota[0] && $quota[1] < 0 ) {
			return esc_html( sprintf(
				/* translators: 1: Number of images */
				_n( '%1$d image credit remaining.', '%1$d image credits remaining.', abs( $quota[1] ), 'ewww-image-optimizer-cloud' ), abs( $quota[1] )
			) );
			...
			return esc_html( sprintf(
				...
				_n( 'used %1$d of %2$d, usage will reset in %3$d day.', 'used %1$d of %2$d, usage will reset in %3$d days.', $quota[2], 'ewww-image-optimizer-cloud' ), $quota[1], $quota[0], $quota[2]
			) );
		}
	}
}
...
function ewww_image_optimizer_options( $network = 'singlesite' ) {
	...
	if ( ! ewww_image_optimizer_full_cloud() ) {
		if ( ewww_image_optimizer_safemode_check() ) {
			$status_output .= 'safe mode: <span style="color: red; font-weight: bolder">' . esc_html__( 'On', 'ewww-image-optimizer-cloud' ) . '</span>&emsp;&emsp;';
			$collapsible    = false;
		} else {
			$status_output .= 'safe mode: <span style="color: green; font-weight: bolder">' . esc_html__( 'Off', 'ewww-image-optimizer-cloud' ) . '</span>&emsp;&emsp;';
		}
	...
	if ( defined( 'EWWW_DISABLE_ASYNC' ) && EWWW_DISABLE_ASYNC ) {
		...
		$status_output .= '<span style="color: orange; font-weight: bolder">' . esc_html__( 'Disabled automatically, async requests blocked', 'ewww-image-optimizer-cloud' ) . '</span>';
	} elseif ( ewww_image_optimizer_detect_wpsf_location_lock() ) {
		$status_output .= '<span style="color: orange; font-weight: bolder">' . esc_html__( "Disabled by Shield's Lock to Location feature", 'ewww-image-optimizer-cloud' ) . '</p>';
